# GLEAM Model: Download and Data Processing
...
## 1. Enter the variable and time window of your interest
...
while (is.na(start_year) || is.na(end_year)) {
  ...
  if (end_year <= start_year) {
    print("Error: End year cannot be equal or smaller than the start year. Please enter valid years.")
    end_year <- as.numeric(readline(prompt = "Enter the end year you are interested in: "))
  }
}
...
for (year in start_year:end_year) {
  ...
  sftp_download(file = sprintf("%s_%s_GLEAM_v3.8a.nc", variable, year), tofolder = user_wd)
}
```

## 4. Processing of GLEAM NetCDF dataset
...
colnames(output_daily) <- c("Day", paste0(variable," [",units,"]"))
...
colnames(output_monthly) <- c("Year", "Month", paste0(variable," [",units,"]"))
...
for (i in 1:length(nc_files)) {
  ...
  year_info <- substr(nc_files[i], nchar(nc_files[i]) - 18, nchar(nc_files[i]) - 15)  #year from file name
  ...
  for (j in 1:nrow(coord_df)) {
    ...
    point_output_daily <- rbind(point_output_daily, point_df)
  }
...
point_output_daily$date <- as.Date(paste(point_output_daily$year, "-", point_output_daily$day, sep = ""), format = "%Y-%j")
point_output_daily <- point_output_daily[, c(1, 2, 6, 5)]

# Rename the columns and days based on the start and end dates
colnames(point_output_daily) <- c("Latitude", "Longitude", "Date", paste0(variable," [",units,"]"))
...
colnames(point_output_monthly) <- c("Latitude", "Longitude", "Year", "Month", paste0(variable," [",units,"]"))
